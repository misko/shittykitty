...
bite=1.8;
hitch_buffer=1.0;
module bearing_hitch(l) {
    translate([0,0,-bearing_width-bearing_nipple]) { 
    difference() {
        difference() {
            union() {
                translate([0,0,-l]) cylinder(l,bearing_inner+hitch_buffer,bearing_inner+hitch_buffer);
                translate([0,0,0]) cylinder(bearing_width,bearing_inner,bearing_inner);
                translate([0,0,bearing_width]) {
                    cylinder(bearing_nipple,bearing_inner+hitch_buffer,bearing_inner);
                }
            }
            
            translate([-(bearing_inner+hitch_buffer),-1,-8+3+bearing_width]) cube([(bearing_inner+hitch_buffer)*2,2,8.1]);
        }   
        
        union() {
            //bite off
            translate([bearing_inner+hitch_buffer-bite,-bearing_inner-hitch_buffer,-l-0.5]) cube([bite,(bearing_inner+hitch_buffer)*2,bearing_width+l+bearing_nipple+1]);
            translate([-bearing_inner-hitch_buffer,-bearing_inner-hitch_buffer,-l-0.5]) cube([bite,(bearing_inner+hitch_buffer)*2,bearing_width+l+bearing_nipple+1]);
...
//distance between  = d
module double_bearing_hitch(d) {
        l=(d-2*(bearing_nipple+bearing_width))/2;
        translate([0,0,l+bearing_width+bearing_nipple]) bearing_hitch(l);
        translate([0,0,-(l+bearing_width+bearing_nipple)]) rotate([0,180,0])  bearing_hitch(l);
}

module carriage_classic(dc) {  
    frame=10;
    frame_height=2*(bearing_inner-bite);
    holder_width=5;
    ...
    difference() { 
        ...
        union() {
            ...
            translate([0,hitch_length/2,0]) rotate([90,90,0]) double_bearing_hitch(hitch_length);
            translate([dc,hitch_length/2,0]) rotate([90,90,0]) double_bearing_hitch(hitch_length);
            translate([0,bearing_width+bearing_nipple,-(bearing_inner-bite)]) cube([dc,frame,frame_height]);
            translate([0,hitch_length-(bearing_width+bearing_nipple)-frame,-(bearing_inner-bite)]) cube([dc,frame,frame_height]);
...
module servo(w=20,h=40,l=40) {
    translate([-l/2,-w/2,-h/2]) {
        translate([w/2,w/2,h]) cylinder(5,5,5);
        cube([l,w,h]);   
    }
}
...
module carriage(dc) {  
    ...
    color([0,1,0]) difference() { 
        ...
        union() {
            //single hitch easy
            translate([0,0,0]) rotate([90,90,0]) bearing_hitch(dc+frame);
...
//spur_gear (modul=1, tooth_number=35, width=5, bore=4, pressure_angle=20, helix_angle=0, optimized=true);
